...
class Glove(object):
    ...
    def _similarity_query(self, word_vec, number):
        ...
        results = []
        for i in word_ids[:number]:
            if i in self.inverse_dictionary:
                results.append((self.inverse_dictionary[i], dst[i]))
        return results
        # return [(self.inverse_dictionary[x], dst[x]) for x in word_ids[:number] if x in self.inverse_dictionary]
    ...
    def most_similar(self, word, number=5):
        ...
        print("Requesting similarity query...")
    ...
    def get_word_vector(self, word):
        try:
            ...
            raise Exception('Word \"{}\"not in dictionary').format(word)
