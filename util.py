...
def getWordSimilarity(w1, w2):
    return ppp.computeVectorDistance(ppp.getWordVector(w1), ppp.getWordVector(w2)) 
...
class ShittySearch(SearchProblem):
    ...
    def succAndCost(self,state):
        results = []
        eval_word = state[0]
        synonyms = thesaurus.browseWord(eval_word)
        for syn in list(synonyms):
            results.append(syn, (syn,state[1]+1), self.costFunc)
        return results

def segmentWords(query, costFunc):
    if len(query) == 0:
        return ''

    ucs = util.UniformCostSearch(verbose=0)
    ucs.solve(SegmentationProblem(query, getWordSimilarity))
